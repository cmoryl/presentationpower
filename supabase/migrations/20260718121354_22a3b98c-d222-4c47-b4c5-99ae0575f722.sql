...
CREATE INDEX IF NOT EXISTS decks_is_template_idx ON public.decks(is_template) WHERE is_template = true;

-- Allow any authenticated user to SELECT decks flagged as templates.
DROP POLICY IF EXISTS "Authenticated can read templates" ON public.decks;
CREATE POLICY "Authenticated can read templates"
  ON public.decks FOR SELECT
  TO authenticated
  USING (is_template = true);

-- RPC: return deck + slides for a template deck, else null.
...
DECLARE
  ...
  _brief jsonb;
BEGIN
  IF _deck_id IS NULL THEN RETURN NULL; END IF;
