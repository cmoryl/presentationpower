...
export function StyleBriefPanel({
  ...
  const pick =
    <K extends keyof StyleIntentBrief>(key: K) =>
    (v: string) =>
  ...
  return (
    <div className="rounded-lg border border-black/10 bg-[#F2F2F2]/60 p-2 dark:border-white/10 dark:bg-white/[0.03]">
      <p className="mb-2 text-[10px] text-[#03002C]/50 dark:text-white/50">
        Answer the brief and the catalog ranks itself — intent, then audience, then story, then
        style.
      </p>
      <div className="grid grid-cols-2 gap-2 sm:grid-cols-3 lg:grid-cols-4">
        <Field
          ...
          onChange={pick("objective")}
          options={entries(OBJECTIVE_LABELS)}
        />
        <Field
          label="Audience"
          value={brief.audience ?? ""}
          onChange={pick("audience")}
          options={entries(AUDIENCE_LABELS)}
        />
        <Field
          label="Slide job"
          value={brief.slideJob ?? ""}
          onChange={pick("slideJob")}
          options={entries(SLIDE_JOB_LABELS)}
        />
        <Field
        ...
        <Field
          label="Output"
          value={brief.output ?? ""}
          onChange={pick("output")}
          options={entries(OUTPUT_LABELS)}
        />
