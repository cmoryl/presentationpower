...
export function KpiDashboardPortrait({
  ...
  return (
    <section aria-label={section.title ?? "KPI dashboard"} style={{ margin: `${cq(18)} 0` }}>
      ...
      <div className="grid" style={{ gridTemplateColumns: `repeat(${cols}, minmax(0, 1fr))`, columnGap: cq(24), rowGap: cq(20) }}>
        {items.map((it, i) => {
          ...
          return (
            ...
            >
              <div style={{ display: "flex", alignItems: "baseline", gap: cq(4) }}>
                <span style={{ fontSize: cq(46), fontWeight: 700, lineHeight: 0.95, letterSpacing: "-0.035em", color: ink.strong, fontVariantNumeric: "tabular-nums" }}>
                  {it.value || "—"}
                </span>
                {it.unit && (
                  <span style={{ fontSize: cq(16), fontWeight: 600, color: accent, letterSpacing: "-0.015em" }}>{it.unit}</span>
                )}
              </div>
              <div style={{ marginTop: cq(6), fontSize: cq(10.5), lineHeight: 1.35, color: ink.soft, maxWidth: cq(220) }}>
                {it.label}
              </div>
              {it.delta && (
                <div style={{ marginTop: cq(4), fontSize: cq(9), fontWeight: 600, color: accent, letterSpacing: "0.02em" }}>
                  {trendUp ? "▲" : "▼"} <span style={{ fontVariantNumeric: "tabular-nums" }}>{it.delta}</span>
