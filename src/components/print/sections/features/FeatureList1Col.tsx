    ...
    <section aria-label={section.title ?? "Features"} style={{ margin: `${cq(16)} 0` }}>
      ...
      <div>
        {items.map((f, i) => {
          ...
          return (
            <div key={i} className="flex items-start" style={{ gap: cq(14), padding: `${cq(12)} 0`, borderTop: i === 0 ? "none" : `1px solid ${ink.hairline}` }}>
              <div
                className="flex items-center justify-center"
                style={{
                  width: cq(32), height: cq(32), borderRadius: cq(8),
              ...
              <div style={{ flex: 1 }}>
                ...
                {f.body && (
                  <div style={{ marginTop: cq(3), fontSize: cq(10), lineHeight: 1.5, color: ink.soft, ...clampLines(3) }}>{f.body}</div>
                )}
              </div>
            </div>
          );
