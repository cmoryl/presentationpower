  ...
  return (
    <section aria-label={section.title ?? "Features"} style={{ margin: `${cq(18)} 0` }}>
      ...
      <div style={{ borderRadius: cq(16), padding: cq(16), ...sectionGlass(mode, accent) }}>
        <div className="grid" style={{ gridTemplateColumns: `repeat(${cols}, minmax(0, 1fr))`, gap: cq(14) }}>
          {items.map((f, i) => {
            const name = (f.icon as IconName) && ICON_PATHS[f.icon as IconName] ? (f.icon as IconName) : VERB_FALLBACK[i % VERB_FALLBACK.length]!;
            return (
              <div key={i} style={{ padding: `${cq(10)} ${cq(4)}` }}>
                <div
                  className="flex items-center justify-center"
                  style={{
                    width: cq(36), height: cq(36), borderRadius: cq(10),
                ...
                <div style={{ marginTop: cq(10), fontSize: cq(13), fontWeight: 700, color: ink.strong, letterSpacing: "-0.005em" }}>
                  {f.verb}
                </div>
                {f.body && (
                  <div style={{ marginTop: cq(4), fontSize: cq(10), lineHeight: 1.5, color: ink.soft, ...clampLines(4) }}>
                    {f.body}
                  </div>
                )}
              </div>
