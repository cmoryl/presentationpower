...
function Cell({ item, mode }: { item: PrintLogoItem; mode: "light" | "dark" }) {
  ...
  return (
    <div style={{ display: "flex", alignItems: "center", justifyContent: "center", height: cq(46) }}>
      {url ? (
        <img src={url} alt={item.name} style={{ maxHeight: "70%", maxWidth: "88%", objectFit: "contain", filter: mode === "dark" ? "brightness(0) invert(1)" : undefined, opacity: 0.85 }} />
      ) : (
    ...
    <section aria-label={section.title ?? "Client logos"} style={{ margin: `${cq(16)} 0` }}>
      {(section.eyebrow || section.title) && (
        <div className="flex items-center" style={{ gap: cq(14), marginBottom: cq(10) }}>
          ...
          <div style={{ fontSize: cq(9.5), fontWeight: 700, letterSpacing: "0.18em", color: accent, textTransform: "uppercase" }}>
            {section.title || section.eyebrow}
          </div>
          <div style={{ flex: 1, height: 1, background: ink.hairline }} />
        </div>
      )}
      <div className="grid" style={{ gridTemplateColumns: `repeat(${items.length}, minmax(0, 1fr))`, gap: 0 }}>
        {items.map((it, i) => (
