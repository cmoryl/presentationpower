  ...
  return (
    <section aria-label={section.title ?? "We know how"} style={{ margin: `${cq(20)} 0` }}>
      <div className="flex items-center" style={{ gap: cq(14), marginBottom: cq(18) }}>
        ...
        <div style={{ fontSize: cq(10), fontWeight: 700, letterSpacing: "0.16em", color: accent, textTransform: "uppercase" }}>
          {section.title || section.eyebrow || "We know how"}
        </div>
        <div style={{ flex: 1, height: 1, background: ink.hairline }} />
      </div>
      <div className="grid" style={{ gridTemplateColumns: `repeat(${items.length}, minmax(0, 1fr))`, gap: cq(12), textAlign: "center" }}>
        {items.map((it, i) => {
          const name = (it.icon as IconName) && ICON_PATHS[it.icon as IconName] ? (it.icon as IconName) : FALLBACK[i % FALLBACK.length]!;
          return (
            <div key={i} className="flex flex-col items-center" style={{ gap: cq(8) }}>
              ...
              <div style={{ fontSize: cq(9.5), lineHeight: 1.4, color: ink.soft, ...clampLines(3) }}>{it.label}</div>
            </div>
          );
        })}
      </div>
