...
function TransPerfectWordmark({ height }) {
    const width = height * (432 / 44.4);
    return (<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 432 44.4" width={width} height={height} fill="currentColor" aria-hidden style={{ display: "block" }}>
...
export function BrandLockup({ brand, color, size = "md", showMark = true, showDivision = true, clientName, }) {
    const dims = size === "xs"
    ...
    return (<div className="flex items-center" style={{ gap: dims.gapPx, color }} role="img" aria-label={`${logo.wordmark}${divisionLine ? " — " + divisionLine : ""} lockup`}>
      {showMark && !useOfficialWordmark && (<div className="flex items-center justify-center font-semibold tracking-tight" style={{
                width: dims.markPx,
                height: dims.markPx,
      ...
      <div className="flex flex-col leading-none">
        {useOfficialWordmark ? (<TransPerfectWordmark height={wordmarkHeight}/>) : (<div className="font-semibold tracking-wide" style={{ fontSize: dims.wordPx, letterSpacing: "0.02em" }}>
            {logo.wordmark.toUpperCase()}
          </div>)}
        {showDivision && divisionLine && (<div className="uppercase tracking-[0.2em] opacity-70" style={{ fontSize: dims.dividerPx, marginTop: useOfficialWordmark ? 6 : 4 }}>
            {divisionLine}
          </div>)}
      </div>
    </div>);
