...
export function SlideFrame({
  ...
  const isElementSkin = ["S29", "S30"].includes(skinCodeFromPackId(String(pack?.id ?? "")));
  const brand = isElementSkin ? ELEMENT_BRAND_MODE : brandProp;
    ...
    >
      ...
      {packOwnsGround &&
        ...
        (() => {
          ...
          // authored-plate probe and the AI backdrop all paint the SAME scene:
          // the module's own vocabulary (stats / chart / bento / timeline /
          // split / quote), with the layout id riding along only as jitter.
      ...
      {!hasBackdrop && !packOwnsGround && (enterprise || !slideDark) && !enterpriseDark && (
        <>
          ...
          <div
            aria-hidden
            data-decorative="true"
            data-brand-system-ground={brandSystemLightGround(groundSeed, brand.tokens.accent).slice(0, 80) || "EMPTY"}
            className="pointer-events-none absolute inset-0"
