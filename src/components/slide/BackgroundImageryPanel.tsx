...
export function BackgroundImageryPanel({
  ...
  const brandResults = useMemo(() => {
    ...
    return rows.filter((r) => {
      const hay =
        `${r.filename} ${r.note ?? ""} ${r.prompt ?? ""} ${(r.tags ?? []).join(" ")}`.toLowerCase();
        ...
        <>
                ...
                <div className="mt-3 text-xs text-black/70">
                  <div>
                    ...
                    {sectionSlides.length > 1 &&
                      " Other slides in this section will inherit the current background."}
                  </div>
                  ...
                  <div className="flex items-center justify-between gap-2">
                    ...
                    <span className="whitespace-nowrap text-[10px] uppercase tracking-widest text-black/40">
                      {brandQuery.isLoading
                        ? "Loading…"
                        : `${brandResults.length} of ${(brandQuery.data ?? []).length}`}
                    </span>
                  </div>
                  {brandQuery.data && brandQuery.data.length === 0 && !brandQuery.isLoading && (
                    <div className="rounded-xl border border-dashed border-black/15 px-4 py-6 text-center text-xs text-black/60">
                      ...
                      <div className="mt-1 text-[11px] text-black/40">
                        Add uploads in Admin · Knowledge · Imagery.
                      </div>
                    </div>
                  )}
          ...
          {tab === "solid" && (
            <div className="mt-4 space-y-4">
              <div
                className="aspect-[16/6] w-full rounded-xl border border-black/10"
                style={{ background: solidPreview }}
              />
              <ColorField
                label="Color"
                value={solidColor}
                onChange={(c) => commitSolid({ color: c })}
              />
              <Slider
                label="Intensity"
            ...
            <div className="mt-4 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                <ColorField
                  label="Color A"
                  value={gradA}
                  onChange={(c) => commitGradient({ color: c })}
                />
                <ColorField
                  label="Color B"
                  value={gradB}
                  onChange={(c) => commitGradient({ colorB: c })}
                />
              </div>
              <Slider
                label="Angle"
          ...
          {tab === "pattern" && (
            <div className="mt-4 space-y-4">
              <div
                className="aspect-[16/6] w-full rounded-xl border border-black/10"
                style={{ background: patternPreview }}
              />
              <div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <ColorField
                  label="Foreground"
                  value={patFg}
                  onChange={(c) => commitPattern({ color: c })}
                />
                <ColorField
                  label="Background"
                  value={patBg}
                  onChange={(c) => commitPattern({ colorB: c })}
                />
              </div>
              <Slider
                label="Intensity"
          ...
          {(current?.kind === "upload" || current?.kind === "ai") && (
            <div className="mt-4 space-y-4 border-t border-black/10 pt-4">
              ...
              <button
                type="button"
                onClick={() =>
                  onChange({ ...current, zoom: 1, offsetX: 0, offsetY: 0, fit: "cover" })
                }
