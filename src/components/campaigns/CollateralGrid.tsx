...
export function CollateralGrid({ items }: { items: PlaybookDeliverable[] }) {
  // Group by category, preserving canonical order first, then anything unknown.
  ...
  const liveCount = items.filter((d) => (d.status ?? "live") === "live").length;
  const soonCount = items.length - liveCount;
    ...
    <div className="space-y-8">
      <div className="flex flex-wrap items-center gap-3 text-[11px] text-black/55">
        <span className="inline-flex items-center gap-1 rounded-full border border-[#A6FA87]/40 bg-[#A6FA8722] px-2 py-0.5 font-semibold uppercase tracking-widest text-[#2F6D1B]">
          <CircleCheck size={10} /> {liveCount} live
        </span>
        <span className="inline-flex items-center gap-1 rounded-full border border-black/10 bg-white/85 px-2 py-0.5 font-semibold uppercase tracking-widest text-black/55">
          <Clock size={10} /> {soonCount} coming soon
        </span>
        ...
        return (
          <div key={cat}>
            <div className="mb-3 flex items-baseline justify-between">
              ...
              <span className="text-[11px] text-black/45">{rows.length} pieces</span>
            </div>
            <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
              {rows.map((d, i) => {
                const isSoon = (d.status ?? "live") === "coming-soon";
                return (
                  <div
                    key={`${d.label}-${i}`}
                  ...
                  >
                    ...
                    <div className="mt-1">
                      <div className={`text-sm font-semibold ${isSoon ? "text-black/70" : "text-[#03002C]"}`}>
                        {d.label}
                      </div>
