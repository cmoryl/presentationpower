...
import { DEFAULT_CANVAS_GRADIENT, canvasFillCss, type CanvasGradientFill } from "@/lib/canvas-fill";
...
export function StudioInspector({
  ...
  if (!item)
    return (
      <div
        className={`w-[300px] shrink-0 rounded-2xl border border-black/10 bg-white/80 p-4 text-xs leading-relaxed text-black/50 backdrop-blur dark:border-white/10 dark:bg-white/[0.04] dark:text-white/50 ${className}`}
      >
  ...
  return (
    <div
      className={`flex w-[300px] shrink-0 flex-col gap-3 overflow-y-auto rounded-2xl border border-black/10 bg-white/80 p-4 backdrop-blur dark:border-white/10 dark:bg-white/[0.04] ${className}`}
    >
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-1">
          <button
            type="button"
            onClick={onDuplicate}
            className="rounded px-2 py-1 text-[11px] font-semibold hover:bg-black/5 dark:hover:bg-white/10"
          >
            Duplicate
          </button>
          <button
          ...
          <Row key={k} label={k.toUpperCase()}>
            <input
              ...
              onChange={(e) => {
                ...
                onPatch({
                  [k]: Math.max(k === "w" || k === "h" ? 40 : 0, Math.min(max, v)),
        ...
        <>
          ...
          <Row label="Fit">
            <select
              ...
              value={item.fit}
              onChange={(e) =>
                onPatch({ fit: e.target.value as "cover" | "contain" } as Partial<CanvasItem>)
              }
            >
          ...
          <Row label="Mode override">
            <select
              ...
              onChange={(e) =>
                onPatch({
                  mode: (e.target.value || undefined) as "light" | "dark" | undefined,
                } as Partial<CanvasItem>)
              }
            >
          ...
          <div className="grid grid-cols-2 gap-2">
            <Row label="Offset X">
              <input
                ...
                onChange={(e) =>
                  onPatch({ offsetX: Number(e.target.value) || 0 } as Partial<CanvasItem>)
                }
              />
            </Row>
            <Row label="Offset Y">
              <input
                ...
                onChange={(e) =>
                  onPatch({ offsetY: Number(e.target.value) || 0 } as Partial<CanvasItem>)
                }
              />
            </Row>
          </div>
          ...
          <div className="grid grid-cols-2 gap-2">
            <Row label="Size">
              <input
                ...
                onChange={(e) =>
                  onPatch({
                    size: Math.max(12, Number(e.target.value) || 12),
                  } as Partial<CanvasItem>)
                }
              />
            </Row>
            <Row label="Weight">
              <select
                ...
                value={item.weight}
                onChange={(e) =>
                  onPatch({ weight: Number(e.target.value) as 400 } as Partial<CanvasItem>)
                }
              >
          ...
          <div className="grid grid-cols-2 gap-2">
            ...
            <Row label="Surface">
              <select
                ...
                value={item.surface}
                onChange={(e) =>
                  onPatch({ surface: e.target.value as "plate" } as Partial<CanvasItem>)
                }
              >
              ...
              <div className="grid grid-cols-2 gap-2">
                <Row label="From">
                  <input
                    ...
                    onChange={(e) =>
                      onPatch({
                        gradient: { ...grad(item), from: e.target.value },
                      } as Partial<CanvasItem>)
                    }
                  />
                </Row>
                <Row label="To">
                  <input
                    ...
                    onChange={(e) =>
                      onPatch({
                        gradient: { ...grad(item), to: e.target.value },
                      } as Partial<CanvasItem>)
                    }
                  />
                </Row>
