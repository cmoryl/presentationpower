...
const EventInput = z.object({
  ...
  slideId: z.string().max(64).nullable().optional(),
  ...
  .handler(async ({ data, context }) => {
    ...
    const [
      ...
      s
        .from("usage_events")
        .select(
          "user_id, session_id, event_type, event_category, division_id, deck_id, slide_id, variant_id, module_family, surface, duration_ms, value, props, created_at",
        )
        .gte("created_at", from)
      ...
      s
        .from("usage_events")
        .select("user_id, event_type, event_category, created_at", { count: "exact", head: true })
        .gte("created_at", priorFrom)
        .lt("created_at", from),
      s
        .from("ai_events")
        .select(
          "user_id, brand_id, surface, model, operation, status, tokens_in, tokens_out, cost_credits, latency_ms, created_at",
        )
        .gte("created_at", from)
        .order("created_at", { ascending: false })
        .limit(10000),
      s
        .from("imagery_events")
        .select("event_type, brand_id, user_id, created_at")
        .gte("created_at", from)
        .limit(10000),
      s
        .from("decks")
        .select("id, title, owner_id, brand_mode_id, status, created_at, updated_at")
        .order("updated_at", { ascending: false })
        .limit(2000),
      s
        .from("deck_share_views")
        .select("deck_id, slides_viewed, max_slide_reached, viewed_at, session_key")
        .gte("viewed_at", from)
        .limit(10000),
      s
        .from("briefs")
        .select("id, owner_id, brand_mode_id, created_at")
        .gte("created_at", from)
        .limit(5000),
      s
        .from("print_assets")
        .select("id, kind, brand_mode_id, owner_id, created_at, updated_at")
        .gte("created_at", from)
        .limit(2000),
      s
        .from("deck_translations")
        .select("source_lang, target_lang, status, created_at")
        .gte("created_at", from)
        .limit(5000),
      s.from("profiles").select("id, display_name").limit(5000),
    ]);

    const usage = ((usageRes as { data?: UsageRow[] }).data ?? []).filter(
      (r) => !data.divisionId || r.division_id === data.divisionId,
    );
    const ai = ((aiRes as { data?: AiRow[] }).data ?? []).filter(
      (r) => !data.divisionId || r.brand_id === data.divisionId,
    );
    const imgs = ((imgRes as { data?: ImgRow[] }).data ?? []).filter(
      (r) => !data.divisionId || r.brand_id === data.divisionId,
    );
    const decks = ((decksRes as { data?: DeckRow[] }).data ?? []).filter(
      (r) => !data.divisionId || r.brand_mode_id === data.divisionId,
    );
    const shares = (sharesRes as { data?: ShareRow[] }).data ?? [];
    const briefs = ((briefsRes as { data?: BriefRow[] }).data ?? []).filter(
      (r) => !data.divisionId || r.brand_mode_id === data.divisionId,
    );
    const prints = ((printRes as { data?: PrintRow[] }).data ?? []).filter(
      (r) => !data.divisionId || r.brand_mode_id === data.divisionId,
    );
    ...
    const uniqueShareSessions = new Set(
      shares.map((v) => v.session_key ?? Math.random().toString()),
    ).size;
    ...
    for (const r of usage) {
      ...
      const entry = variantCounts.get(r.variant_id) ?? {
        renders: 0,
        adds: 0,
        edits: 0,
        exports: 0,
      ...
      if (r.division_id)
        entry.division.set(r.division_id, (entry.division.get(r.division_id) ?? 0) + 1);
    ...
    for (const r of usage)
      if (r.module_family)
        familyCounts.set(r.module_family, (familyCounts.get(r.module_family) ?? 0) + 1);
    ...
    const divisionAgg = new Map<
      string,
      { events: number; decks: number; users: Set<string>; ai: number; exports: number }
    >();
    for (const r of usage) {
      const key = r.division_id ?? "—";
      const entry = divisionAgg.get(key) ?? {
        events: 0,
        decks: 0,
        users: new Set(),
        ai: 0,
    ...
    const userAgg = new Map<
      string,
      { events: number; decks: number; ai: number; exports: number; lastSeen: string }
    >();
    for (const r of usage) {
      if (!r.user_id) continue;
      const e = userAgg.get(r.user_id) ?? {
        events: 0,
        decks: 0,
        ai: 0,
        exports: 0,
    ...
    const powerUsers = Array.from(userAgg.entries())
      .map(([user_id, v]) => ({
        user_id,
        name: nameById.get(user_id) ?? user_id.slice(0, 8),
        ...v,
      }))
      .sort(
        (a, b) =>
          ...
          b.exports * 3 +
          b.decks * 5 -
          (a.events + a.ai * 2 + a.exports * 3 + a.decks * 5),
      )
      .slice(0, 20);
    ...
    const shareByDeck = new Map<
      string,
      { views: number; maxSlide: number; sessions: Set<string> }
    >();
    ...
    const topSharedDecks = Array.from(shareByDeck.entries())
      .map(([deck_id, v]) => ({
        deck_id,
        title: deckTitleById.get(deck_id) ?? deck_id.slice(0, 8),
        views: v.views,
        uniqueViewers: v.sessions.size,
        maxSlide: v.maxSlide,
      }))
    ...
    for (const t of trans)
      transByTarget.set(t.target_lang, (transByTarget.get(t.target_lang) ?? 0) + 1);

    // ── Imagery snapshot ─────────────────────────────────────────────
    let imgGen = 0,
      imgUse = 0;
    for (const r of imgs) {
      if (r.event_type === "generate") imgGen++;
      if (r.event_type === "use") imgUse++;
    }

    // ── WoW delta ─────────────────────────────────────────────────────
    const currentTotal = usage.length;
    const deltaPct =
      priorTotal > 0 ? Math.round(((currentTotal - priorTotal) / priorTotal) * 100) : null;
    ...
    return {
      ...
      topSharedDecks,
      ai: {
        byModel: Array.from(aiByModel.entries())
          .map(([model, v]) => ({
            model,
            calls: v.calls,
        ...
        byOperation: Array.from(aiByOp.entries())
      ...
      translation: {
        ...
        topTargets: Array.from(transByTarget.entries())
