...
/**
 * Directory categories. Rooms sit white on the walkway; circulation and
 * structure stay flat and grey so the routes read as negative space.
 */
...
function markerGlyph(
  ...
  const ink = active ? "#C4306E" : (LONDON_KIND_INK[m.kind] ?? BLUE);
  const r = active ? 9.5 : numbered ? 8.5 : 7.25;
  const tail = r * 1.6;
  const body =
...
function indexBlock(markers: LondonMarker[], x: number, y: number, w: number): string {
  ...
  const entries = markers
    .map((m, i) => {
      ...
      return `<text x="${n(cx + 15)}" y="${n(cy)}" text-anchor="end" font-family="${FONT}" font-size="8.5" font-weight="700" fill="${
...
function legendRow(kinds: LondonAssetKind[], x: number, y: number, w: number): string {
  ...
  for (const chip of chips) {
    ...
    const cy = y + row * 19;
    out.push(
      `<circle cx="${n(cx + 5)}" cy="${n(cy - 3)}" r="4" fill="${LONDON_KIND_INK[chip.k] ?? BLUE}" />` +
...
function planBody(plan: LondonFloorPlan, ox: number, oy: number, roomsOnly = false): string {
  ...
  const grid: string[] = [];
  for (let i = 1; i < plan.w; i += 1) {
    const x = ox + i * PPM;
    grid.push(
      `<path d="M ${n(x)} ${n(oy)} V ${n(oy + ph)}" stroke="${PAPER}" stroke-opacity="${i % 5 === 0 ? 0.85 : 0.4}" stroke-width="1" />`,
    );
  }
  for (let i = 1; i < plan.h; i += 1) {
    const y = oy + i * PPM;
    grid.push(
      `<path d="M ${n(ox)} ${n(y)} H ${n(ox + pw)}" stroke="${PAPER}" stroke-opacity="${i % 5 === 0 ? 0.85 : 0.4}" stroke-width="1" />`,
    );
  }
