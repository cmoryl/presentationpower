...
export const CREATE_ONLY_AGENT_PROMPT = [
  ...
  "- Slide decks and modules for this user are Enterprise mode only: style pack skin-s06 (Enterprise · Light) or skin-s04 (Enterprise · Dark). They may choose light or dark, and may switch the division lockup and copy within that look — divisions no longer carry their own accent colours; every division renders in the approved enterprise palette. Custom templates are only usable when an admin has explicitly approved them for sales; never propose or apply any other style pack.",
