...
function source(bullets: string[]): ParsedSlide {
  return {
    index: 1,
    title: "Four proof stats",
    bullets,
    notes: "",
    images: [],
    charts: [],
...
describe("completeCoverage", () => {
  it("designs continuation pages until every imported line lands on a canvas", () => {
    ...
    const mapped: MappedSlide[] = [mapParsedSlide(source(bullets), 2)];
    const designed = designReinterpretedDeck(mapped, {});
    const covered = completeCoverage(designed);
    const parent = covered[0];
    expect((parent.continuations ?? []).length).toBeGreaterThan(0);
    expect(parent.coverage!.dropped.length).toBeLessThan(designed[0].coverage!.dropped.length);
    const flat = flattenContinuations(covered);
    expect(flat.length).toBe(1 + (parent.continuations ?? []).length);
