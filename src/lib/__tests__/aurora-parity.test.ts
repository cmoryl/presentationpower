...
const brands: BrandMode[] = [
  {
    id: "corporate",
    name: "corporate",
    description: "",
    tokens: { primary: "#003FC7", accent: "#003FC7", surface: "#FFFFFF", ink: "#0A0A0A" },
  },
  {
    id: "life-sciences",
    name: "life-sciences",
    description: "",
    tokens: { primary: "#0A2540", accent: "#00A896", surface: "#F5FBF9", ink: "#0A0A0A" },
  },
  {
    id: "legal",
    name: "legal",
    description: "",
    tokens: { primary: "#1A1A1A", accent: "#C9A46A", surface: "#FAF7F1", ink: "#0A0A0A" },
  },
  {
    id: "media-tech",
    name: "media-tech",
    description: "",
    tokens: { primary: "#0B1330", accent: "#EC388A", surface: "#FFF7FB", ink: "#0A0A0A" },
  },
  {
    id: "regional",
    name: "regional",
    description: "",
    tokens: { primary: "#03002C", accent: "#A1FBF9", surface: "#EEFCFC", ink: "#0A0A0A" },
  },
  {
    id: "trial-interactive",
    name: "trial-interactive",
    description: "",
    tokens: { primary: "#0A2540", accent: "#A6FA87", surface: "#F4FBEE", ink: "#0A0A0A" },
  },
];
...
describe("aurora parity: on-screen renderer ↔ PPTX export", () => {
  it("embeds every orb's color/position/size/alpha from auroraOrbs() into the exported SVG", () => {
    for (const brand of brands) {
      for (const mode of ["dark", "light"] as const) {
        ...
        const svg = decodeSvg(auroraSvgDataUrl(seed, brand, mode));
  ...
  it("bakes the same base tint and layer opacity the renderer applies", () => {
    for (const brand of brands) {
      for (const mode of ["dark", "light"] as const) {
        ...
        const svg = decodeSvg(auroraSvgDataUrl(seed, brand, mode));
        const base = auroraBaseTint(brand, mode);
  ...
  it("uses the mode-specific blur radius and orb radius that AuroraLayer paints (free-form v2 — no vignette, no wash)", () => {
    const dark = decodeSvg(auroraSvgDataUrl("x", brands[0], "dark"));
    const light = decodeSvg(auroraSvgDataUrl("x", brands[0], "light"));
  ...
  it("is deterministic: identical (seed, brand, mode) → identical SVG payload", () => {
    for (const brand of brands) {
      for (const mode of ["dark", "light"] as const) {
        const a = auroraSvgDataUrl("deck-42", brand, mode);
        const b = auroraSvgDataUrl("deck-42", brand, mode);
        expect(a).toBe(b);
      }
    }
  });
  ...
  it("covers every module variant with a valid dark-mode backdrop (no NaN, no missing orbs)", () => {
    ...
    for (const variant of MODULE_VARIANTS.slice(0, 40)) {
      const orbs = auroraOrbs(variant.id, brand, "dark");
      expect(orbs).toHaveLength(3);
      for (const o of orbs) {
