...
describe("QEII Illustrator map export", () => {
  ...
  it("writes a live-vector PDF with named layers and editable text", async () => {
    const res = await buildQeiiPlanAi(floor, { showUse: true });
  ...
  it("keeps a renamed room's corrected name in the file", async () => {
    const room = qeiiPlanLayout(floor).blocks[0]!.room;
    const res = await buildQeiiPlanAi(floor, {
