...
export const CASE_STUDIES: CaseStudy[] = [
  ...
  },
  {
    ...
    challenge:
      "Localized 4,000+ regulated documents / year across 28 markets with fragmented reviewer workflows.",
  ...
  },
  {
    ...
    challenge:
      "Needed instruction-tuned datasets in 42 languages with human preference ranking at frontier scale.",
    solution:
      "DataForce sourced 6,200 specialist annotators, ran calibrated preference workflows, and delivered rolling weekly batches.",
    ...
    quote:
      "Every batch we shipped moved eval scores. We stopped hunting for annotators and started shipping models.",
    ...
    stats: [
      {
        value: "3.1",
        unit: "M",
        label: "ranked preference pairs",
        source: "Program dashboard, 2025",
      },
  ...
  },
  {
    ...
    challenge:
      "Weekly product releases in 18 languages were bottlenecked by manual file exchange with translation teams.",
    solution:
      "GlobalLink connectors into Git and the CMS, with policy-driven auto-routing and in-context review.",
    result:
      "Localization moved from a release-blocker to a background job — 100% coverage on every release.",
    ...
    stats: [
      ...
      { value: "18", unit: "", label: "languages in the pipeline", source: "2025" },
      {
        value: "0",
        unit: "",
        label: "release delays from localization",
        source: "12-month rolling, 2025",
      },
    ],
  },
  {
    ...
    challenge:
      "Client-facing regulatory notices needed identical intent across 22 markets with market-specific counsel review.",
    solution:
      "Single managed program with jurisdiction-aware review paths and one system of record.",
  ...
  },
  {
    ...
    challenge:
      "Seasonal campaign copy fragmented across regional agencies, causing launch-day drift market to market.",
    solution:
      "Central brief, terminology guardrails, and coordinated in-market review — one calendar, one voice.",
  ...
  },
  {
    ...
    challenge:
      "9-jurisdiction antitrust matter with 4.2M documents and rolling productions across four languages.",
    solution:
      "Managed eDiscovery program with linguistic review teams, TAR-assisted culling, and jurisdiction-aware privilege workflows.",
    result:
      "Productions on the court's original schedule, with a single privilege log across every language.",
    ...
    stats: [
      { value: "4.2", unit: "M", label: "documents reviewed", source: "Matter dashboard, 2025" },
      {
        value: "3",
        unit: "×",
        label: "faster than parallel-review baseline",
        source: "Program benchmark, 2025",
      },
  ...
  },
  {
    ...
    challenge:
      "Flagship original needed same-day availability in 32 languages with full access services and no dubbing artifacts.",
    solution:
      "Coordinated dubbing, subtitling, and audio-description production on a single asset pipeline with in-house talent networks.",
    ...
    stats: [
      { value: "32", unit: "", label: "languages dubbed & subtitled", source: "2025" },
      {
        value: "0",
        unit: "",
        label: "premiere delays across the slate",
        source: "Slate report, 2025",
      },
  ...
  },
  {
    ...
    challenge:
      "Global simship with a fixed date, weekly build cadence, and voice recording in 8 languages needed to align to picture.",
    solution:
      "Embedded loc + LQA teams inside the build pipeline, with voice direction and per-locale bug triage.",
    ...
    quote:
      "Locale-specific bugs used to be a launch-week fire. Now they close in the weekly build.",
  ...
  },
  {
    ...
    challenge:
      "Localized storefronts in 12 markets under-indexed on native search terms, capping organic pipeline.",
    solution:
      "Keyword-driven transcreation, native-language content ops, and per-market schema tuning in one program.",
  ...
  },
  {
    ...
    challenge:
      "Multi-country oncology trial stalled in start-up: site activation dragged, and TMF completeness lived in spreadsheets.",
    solution:
      "Trial Interactive eTMF plus investigator portal deployed as the system of record; site activation workflows and reviewer dashboards live from kickoff.",
    result:
      "First patient in 42% faster than protocol baseline; TMF inspection-ready every day of the trial.",
  ...
  },
  {
    ...
    challenge:
      "Two organizations with overlapping content pipelines and different measurement frames.",
    solution:
      "Co-owned program plan, shared dashboard, and quarterly business reviews with joint targets.",
    result:
      "Aligned metrics from day one; joint scorecard replaced the finger-pointing that used to happen at QBRs.",
...
export function pickCaseStudy(brandModeId: string | undefined, industry?: string): CaseStudy {
...
export function pickProofLogos(brandModeId: string | undefined): Array<{ name: string }> {
  ...
  while (base.length < 8)
    base.push({ name: base[base.length % Math.max(base.length, 1)]?.name ?? "Client" });
