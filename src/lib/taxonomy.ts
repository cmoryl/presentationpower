...
export const BRAND_MODES: BrandMode[] = [
  ...
  },
  {
    ...
    // Deep navy field + bright high-key emerald accent. Matches tp-games-dark PNGs
    // and pale-green light backdrops. Aqua sibling is generated by shiftHue in flagship.tsx.
  ...
  },
  {
    ...
    // NEXT Finance peach #FF9B70 — warm partner accent on ink.
...
export const MODULE_VARIANTS: ModuleVariant[] = [
  ...
  // ── Advanced variants — BATCH 1 (structure & data-forward) ──────────────
  {
    ...
    familyId: "MF-04",
    name: "Bento — 5 cell",
    description: "Asymmetric bento grid: one anchor feature + four supporting cells (title, body, stat, media)",
    permittedLayoutIds: ["LF-09", "LF-12"],
    capacity: { items: { min: 5, max: 5 }, titleChars: 80, bodyChars: 200 },
    editableFields: ["title", "items[].kind", "items[].title", "items[].body", "items[].value", "items[].unit", "items[].label", "items[].mediaSeed"],
    lockedFields: ["footer", "logo"],
  },
  // ── MV-VIZ-* · Spec-driven ECharts family (Phase 1 infographic engine) ──
  {
    ...
    familyId: "MF-05",
    name: "Sankey flow",
    description: "Weighted flow diagram from source to target nodes. Ideal for revenue splits, funnel drop-off, workload allocation.",
  ...
  },
  {
    ...
    description: "Circular relationship diagram: bilateral links between entities weighted by volume or affinity.",
    permittedLayoutIds: ["LF-11"],
  ...
  },
  {
    ...
    description: "Non-overlapping dot distribution across one or more categories. Good for benchmarks and score comparisons.",
    permittedLayoutIds: ["LF-11", "LF-14"],
  ...
  },
  {
    ...
    description: "Rank-over-time chart: shows position changes across periods for a small set of competing entities.",
    permittedLayoutIds: ["LF-11", "LF-14"],
  ...
  },
  {
    ...
    description: "Two-axis bubble plot with size + optional category. For competitive maps, portfolio review, opportunity vs. effort.",
    permittedLayoutIds: ["LF-11"],
  ...
  },
  {
    ...
    description: "Nested rectangles sized by value with optional category grouping. For portfolio share and revenue mix.",
    permittedLayoutIds: ["LF-11"],
    capacity: { items: { min: 3, max: 40 }, titleChars: 80 },
    editableFields: ["title", "rows", "encoding", "source"],
    lockedFields: ["footer", "logo"],
  },
  {
    ...
    description: "Daily-value heatmap over a calendar year. Great for activity, incidents, engagement cadence.",
    permittedLayoutIds: ["LF-11"],
  ...
  },
  {
    ...
    familyId: "MF-05",
    name: "KPI dashboard",
    description: "6–8 compact KPIs in a baseline-aligned grid with hairline rules, tiny trend arrows and deltas",
    permittedLayoutIds: ["LF-11", "LF-10"],
    capacity: { items: { min: 6, max: 8 }, titleChars: 80 },
    editableFields: ["title", "items[].value", "items[].unit", "items[].label", "items[].delta", "items[].trend"],
    lockedFields: ["footer", "logo"],
  },
  {
    ...
    description: "Horizontal quarterly roadmap: Q1–Q4 columns with workstream bars spanning quarters",
    permittedLayoutIds: ["LF-14"],
    capacity: { items: { min: 3, max: 6 }, titleChars: 80 },
    editableFields: ["title", "quarters", "items[].label", "items[].start", "items[].end", "items[].note"],
    lockedFields: ["footer", "logo"],
  },
  {
    id: "MV-FUNNEL",
  ...
  },
  {
    ...
    familyId: "MF-04",
    name: "Flywheel",
    description: "Circular flywheel: 4–6 nodes on a thin track with a centre label and momentum arrows",
    permittedLayoutIds: ["LF-34"],
  ...
  },
  {
    ...
    familyId: "MF-04",
    name: "Maturity curve",
    description: "Ascending SVG curve with 4 milestone dots from Today to Target, current position marked",
    permittedLayoutIds: ["LF-14"],
    capacity: { items: { min: 3, max: 5 }, titleChars: 80 },
  ...
  },
  {
    ...
    familyId: "MF-04",
    name: "Journey map",
    description: "Horizontal customer journey: 5 phases with touchpoints and an emotion/effort polyline",
    permittedLayoutIds: ["LF-14"],
  ...
  },
  {
    ...
    editableFields: ["title", "axisX", "axisY", "quadrants", "target", "items[].label", "items[].x", "items[].y"],
    lockedFields: ["footer", "logo"],
  },
  {
    id: "MV-ICEBERG",
    ...
    description: "Above-and-below-waterline metaphor: visible items above a hairline waterline, hidden items below",
    permittedLayoutIds: ["LF-22"],
    capacity: { items: { min: 4, max: 6 }, titleChars: 80 },
    editableFields: ["title", "waterline", "above[].label", "above[].body", "below[].label", "below[].body"],
    lockedFields: ["footer", "logo"],
  },
  ...
  // ── Advanced variants — BATCH 2 (editorial & narrative) ─────────────────
  {
    ...
    description: "Magazine spread — oversized left-column pull-stat or drop-cap, two body columns with hairline column rule, folio meta",
    permittedLayoutIds: ["LF-25"],
    capacity: { titleChars: 90, bodyChars: 900 },
    editableFields: ["title", "kicker", "pullValue", "pullUnit", "pullLabel", "bodyLeft", "bodyRight", "folio"],
    lockedFields: ["footer", "logo"],
  },
  {
    ...
    description: "Asymmetric split — left dark manifesto panel, right light with 3 hairline-topped proof points",
  ...
  },
  {
    ...
    description: "Three monumental stats side-by-side with full-height hairline dividers and short context lines",
    permittedLayoutIds: ["LF-08"],
    capacity: { items: { min: 3, max: 3 }, titleChars: 90 },
    editableFields: ["title", "items[].value", "items[].unit", "items[].label", "items[].note", "items[].source"],
    lockedFields: ["footer", "logo"],
  },
  {
    ...
    description: "Vertical milestone timeline with an accent spine, tabular date, dot nodes, label + short body per entry",
  ...
  },
  {
    ...
    description: "Before/after with strong vertical accent divider + arrow, matched StatFigures, after side visually elevated",
    permittedLayoutIds: ["LF-15"],
    capacity: { titleChars: 90 },
    editableFields: ["title", "before.label", "before.value", "before.unit", "before.body", "after.label", "after.value", "after.unit", "after.body"],
    lockedFields: ["footer", "logo"],
  },
  {
    ...
    description: "Dominant hero quote with quote-mark treatment plus two smaller supporting quotes in a hairline row",
    permittedLayoutIds: ["LF-20", "LF-38"],
    capacity: { items: { min: 2, max: 2 }, titleChars: 240 },
    editableFields: ["hero.quote", "hero.name", "hero.role", "hero.org", "items[].quote", "items[].name", "items[].role", "items[].org"],
    lockedFields: ["footer", "logo"],
  },
  {
    id: "MV-DEFINITION",
    familyId: "MF-03",
    name: "Definition",
    description: "Dictionary-style term, pronunciation meta, italic part-of-speech, large definition and italic usage example",
  ...
  },
  {
    ...
    familyId: "MF-03",
    name: "Principles",
    description: "3–5 numbered principles with oversized accent ordinals, bold statement, one-line supporting text, hairline dividers",
    permittedLayoutIds: ["LF-13"],
  ...
  },
  {
    ...
    description: "Three full-width rows with monumental accent numerals and big statements, hairline dividers",
    permittedLayoutIds: ["LF-13"],
    capacity: { items: { min: 3, max: 3 }, titleChars: 90 },
  ...
  },
  {
    ...
    description: "Three horizontal bands (Now/Next/Later) with hairline dividers and progressively muted ink",
    permittedLayoutIds: ["LF-22"],
    capacity: { items: { min: 3, max: 3 }, titleChars: 90 },
  ...
  // ── Advanced variants — BATCH 3 (dashboard family) ─────────────────────
  {
    ...
    description: "Two stacked stat cards with sparklines plus a balance panel with a hero stat and secondary rows",
    permittedLayoutIds: ["LF-08", "LF-11"],
    capacity: { titleChars: 80 },
    editableFields: ["title", "primary.label", "primary.value", "primary.unit", "primary.series", "secondary.label", "secondary.value", "secondary.unit", "secondary.series", "balance.value", "balance.unit", "balance.label", "balance.items[].label", "balance.items[].value"],
    lockedFields: ["footer", "logo"],
  },
  {
    ...
    description: "Three donut gauges side-by-side with tabular % centered, label + one-line body beneath",
    permittedLayoutIds: ["LF-11"],
  ...
  },
  {
    ...
    description: "Large area/line chart across months with side rail: kicker, headline, primary stat with delta",
    permittedLayoutIds: ["LF-08"],
    capacity: { titleChars: 80 },
    editableFields: ["title", "kicker", "headline", "stat.value", "stat.unit", "stat.label", "stat.delta", "series"],
    lockedFields: ["footer", "logo"],
  },
  {
  ...
  },
  {
    ...
    description: "Vertical bar chart on the left with a big StatFigure and a color-swatch legend on the right",
    permittedLayoutIds: ["LF-08"],
    capacity: { titleChars: 80 },
    editableFields: ["title", "bars[].label", "bars[].value", "highlight", "stat.value", "stat.unit", "stat.label", "legend[].label", "legend[].value"],
    lockedFields: ["footer", "logo"],
  },
  {
    ...
    description: "Two side-by-side report cards with big delta headlines, sparklines, and small-caps meta",
    permittedLayoutIds: ["LF-08"],
  ...
  },
  {
    ...
    description: "Year columns with ascending SVG bars and value headlines; final year emphasized in accent",
    permittedLayoutIds: ["LF-11"],
    capacity: { items: { min: 3, max: 5 }, titleChars: 80 },
  ...
  },
  {
    ...
    description: "Rows with label, big value, delta chip, and a horizontal progress bar with % right-aligned",
    permittedLayoutIds: ["LF-13"],
    capacity: { items: { min: 2, max: 4 }, titleChars: 80 },
    editableFields: ["title", "items[].label", "items[].value", "items[].unit", "items[].delta", "items[].percent"],
    lockedFields: ["footer", "logo"],
  },
  {
    ...
    description: "Monumental headline stat left with a right column of region/market rows, deltas, and progress bars",
    permittedLayoutIds: ["LF-08"],
    capacity: { titleChars: 80 },
    editableFields: ["title", "stat.value", "stat.unit", "stat.label", "items[].label", "items[].delta", "items[].percent"],
    lockedFields: ["footer", "logo"],
  },
  ...
  // ── Advanced variants — BATCH 4 (graph family) ─────────────────────────
  {
    ...
    editableFields: ["title", "kicker", "headline", "items[].year", "items[].value", "items[].unit"],
    lockedFields: ["footer", "logo"],
  },
  {
  ...
  },
  {
    ...
    permittedLayoutIds: ["LF-08"],
    capacity: { items: { min: 3, max: 6 }, titleChars: 80 },
    editableFields: ["title", "items[].label", "items[].value", "items[].unit", "stat.value", "stat.unit", "stat.label"],
    lockedFields: ["footer", "logo"],
  },
  {
  ...
  },
  {
    ...
    editableFields: ["title", "kicker", "headline", "series[].label", "series[].value", "callout.year", "callout.note"],
    lockedFields: ["footer", "logo"],
  },
  {
    ...
    editableFields: ["title", "items[].label", "items[].current", "items[].benchmark", "items[].range"],
    lockedFields: ["footer", "logo"],
  },
  ...
  // ── Advanced variants — BATCH 5 (extended graph family) ───────────────
  {
    ...
    description: "Two or three trend lines over a shared timeline with axis, legend, and a highlighted latest point",
    permittedLayoutIds: ["LF-08"],
  ...
  },
  {
    ...
    permittedLayoutIds: ["LF-08"],
    capacity: { items: { min: 3, max: 8 }, titleChars: 80 },
    editableFields: ["title", "axis.x", "axis.y", "items[].label", "items[].x", "items[].y", "items[].size"],
    lockedFields: ["footer", "logo"],
  },
  {
    id: "MV-GRAPH-HEATMAP",
