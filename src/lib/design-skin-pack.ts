...
const SKIN_TRAITS: Record<string, SkinTraits> = {
  ...
  S26: {
    ...
    mono: GEIST_MONO,
    displayWeight: 600,
    displayTracking: "-0.031em",
    displayTransform: "none",
    displayScale: 1,
    kickerWeight: 500,
    kickerTracking: "0.29em",
    radius: 18,
    surfaceStyle: "glass",
    topBar: false,
    grain: 0.05,
...
const INDUSTRY_TRAITS: Record<string, SkinTraits> = {
  ...
  R23: {
    display: F.archivoBlack,
    body: F.barlow,
    kicker: F.archivoBlack,
    displayWeight: 400,
    displayTracking: "-0.04em",
    displayTransform: "uppercase",
    displayScale: 1.06,
...
function highContrastRoles(skin: DesignSkin, r: ReturnType<typeof roles>) {
  ...
  const lift = (hex: string) =>
    Math.abs(luminance(hex) - luminance(r.surface)) < 0.45 ? mix(hex, ink, 0.55) : hex;
