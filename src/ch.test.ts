...
test("c", () => {
  ...
  for (const v of MODULE_VARIANTS as any[]) { const c = seedContent(v.id, {} as any, "") as any; if (chartFrom(c)) { hit++; continue; }
    const keys = Object.keys(c).filter(k=>numy(c[k])); if (keys.length) miss.push(v.id+":"+keys.map(k=>k+"="+JSON.stringify(c[k]).slice(0,110)).join(" | ")); }
