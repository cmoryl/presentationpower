        ...
        <>
          ...
          <section className="grid gap-6 lg:grid-cols-2">
            <div className="rounded-2xl border border-black/10 bg-white/70 p-5 backdrop-blur">
              ...
              {q.data.byBrand.length === 0 ? (
                <div className="py-6 text-center text-xs text-black/40">No data</div>
              ) : (
                <table className="w-full text-xs">
                  <thead>
                    <tr className="text-left uppercase tracking-widest text-black/50">
                  ...
                  <tbody>
                    {q.data.byBrand.map((b) => (
                      <tr key={b.brand} className="border-t border-black/5">
                        <td className="py-1.5 font-mono">{b.brand}</td>
                        <td>{b.total}</td>
                        <td>{b.generate}</td>
                        <td>{b.use}</td>
                      </tr>
            ...
            <div className="rounded-2xl border border-black/10 bg-white/70 p-5 backdrop-blur">
              ...
              {q.data.topPrompts.length === 0 ? (
                <div className="py-6 text-center text-xs text-black/40">No prompt data</div>
              ) : (
                <ul className="space-y-1.5 text-xs">
                  {q.data.topPrompts.map((p) => (
                    <li
                      key={p.prompt}
                      className="flex items-baseline justify-between gap-3 border-b border-black/5 pb-1.5"
                    >
                      <span className="truncate text-black/70" title={p.prompt}>
                        {p.prompt}
                      </span>
                ...
                <tbody>
                  {q.data.byImage.map((i) => (
                    <tr key={i.image_id} className="border-t border-black/5">
                      ...
                      <td
                        className="max-w-[420px] truncate p-2 text-black/60"
                        title={i.prompt ?? ""}
                      >
                        {i.prompt ?? "—"}
                      </td>
                  ...
                  {q.data.byImage.length === 0 && (
                    <tr>
                      <td colSpan={4} className="p-8 text-center text-black/50">
                        No imagery events yet. The imagery library will log generate/view/use events
                        automatically.
                      </td>
                    </tr>
                  )}
