      ...
      {status.data && (
        ...
        >
          <div>
            ...
            {status.data.model && (
              <div className="text-xs opacity-70">Model: {status.data.model}</div>
            )}
          </div>
      ...
      {q.data && (
        <>
          ...
          <section className="grid gap-6 lg:grid-cols-2">
            <Table
              title="Top models"
              cols={["Model", "Calls", "Errors", "Tokens", "Cost"]}
              rows={q.data.byModel.map((r) => [
                r.model,
                r.calls,
                r.errors,
                ...
                <tbody>
                  {q.data.recent.map((r) => (
                    <tr key={r.id} className="border-t border-black/5">
                      ...
                      <td className="p-2">
                        <span
                          className={`rounded-full px-2 py-0.5 text-[10px] ${r.status === "success" ? "bg-emerald-100 text-emerald-800" : r.status === "error" ? "bg-red-100 text-red-800" : "bg-amber-100 text-amber-800"}`}
                        >
                          {r.status}
                        </span>
                      </td>
                      ...
                      <td
                        className="max-w-[240px] truncate p-2 text-black/60"
                        title={r.prompt_summary ?? ""}
                      >
                        {r.prompt_summary ?? "—"}
                      </td>
                    </tr>
                  ...
                  {q.data.recent.length === 0 && (
                    <tr>
                      <td colSpan={9} className="p-8 text-center text-black/50">
                        No AI events yet in this window. Events log automatically as the app calls
                        the AI gateway.
                      </td>
                    </tr>
                  )}
...
function Table({
  ...
  return (
    <div className="rounded-2xl border border-black/10 bg-white/70 p-5 backdrop-blur">
      ...
      {rows.length === 0 ? (
        <div className="py-6 text-center text-xs text-black/40">No data</div>
      ) : (
        <table className="w-full text-xs">
          <thead>
            <tr>
              {cols.map((c) => (
                <th key={c} className="pb-2 text-left uppercase tracking-widest text-black/50">
                  {c}
                </th>
              ))}
            </tr>
