      ...
      <div className="mx-auto w-full max-w-7xl px-4 py-8 sm:px-6 sm:py-10">
          ...
          <div className="grid gap-8 p-6 sm:p-8 lg:grid-cols-[1.35fr_0.65fr] lg:p-10">
            <div className="min-w-0">
              ...
              <p className="mt-4 max-w-2xl text-base leading-relaxed text-primary-foreground/75">
                A live-demo prep surface mapped directly to the score sheet: sell more, automate and
                innovate, and scale.
              </p>
        ...
        <section className="mt-8 grid gap-4 lg:grid-cols-3">
          {JUDGING_CATEGORIES.map((category) => {
            const completeInCategory = category.criteria.filter(
              (criterion) => checked[criterion.id],
            ).length;
            return (
              <article
                key={category.id}
          ...
          <div className="rounded-2xl border border-border bg-card p-5 text-card-foreground shadow-sm">
            ...
            <div className="mt-5 space-y-4">
              {JUDGING_RUN_OF_SHOW.map((step) => (
                <div
                  key={step.time}
                  className="grid gap-3 rounded-xl border border-border bg-background p-4 sm:grid-cols-[8rem_1fr]"
                >
          ...
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
              ...
              <p className="mt-2 max-w-3xl text-sm leading-relaxed text-muted-foreground">
                Uses the approved Enterprise brand system by default with a deliberate light and
                dark slide mix.
              </p>
            </div>
        ...
        <section className="mt-8 grid gap-4 md:grid-cols-3">
          <Link
            to="/agent"
            className="group rounded-2xl border border-border bg-card p-5 text-card-foreground shadow-sm transition hover:border-primary/35"
          >
            ...
            <p className="mt-2 text-sm leading-relaxed text-muted-foreground">
              Create the live deck from a judging-specific prompt.
            </p>
          </Link>
          <Link
            to="/library"
          ...
          >
            ...
            <p className="mt-2 text-sm leading-relaxed text-muted-foreground">
              Prove scale through reusable, governed slide systems.
            </p>
          </Link>
          <Link
            to="/library/print"
          ...
          >
            ...
            <p className="mt-2 text-sm leading-relaxed text-muted-foreground">
              Connect the story to print, social, and event production.
            </p>
          </Link>
        </section>
      </div>
