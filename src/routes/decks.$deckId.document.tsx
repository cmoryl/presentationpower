          ...
          <div className="flex items-start justify-between gap-6">
            <div>
              ...
              <p className="mt-1 text-sm text-black/60">
                Re-flow this deck into a print-ready document. Use your browser's print dialog to
                save as PDF.
              </p>
            </div>
            <button
            ...
            {DOCUMENT_FAMILIES.map((f) => (
              <button
                ...
                className={`rounded-2xl border p-5 text-left transition ${
                  f.id === familyId
                    ? "border-[#0B2A4A] bg-white ring-2 ring-[#0B2A4A]/20"
          ...
          {pages.map((chunk, pageIdx) => (
            <DocumentPage key={pageIdx} dims={dims}>
              <div className="flex h-full flex-col">
                ...
                <div
                  className={`grid flex-1 gap-[0.3in] p-[0.4in] ${chunk.length === 2 ? "grid-rows-2" : "grid-rows-1"}`}
                >
                  {chunk.map((slide, k) => {
