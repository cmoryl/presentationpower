...
function PaletteShowcaseCard() {
  return (
    <Link
      to="/decks/next-palette"
      className="group relative block overflow-hidden rounded-2xl border border-black/10 bg-white transition hover:-translate-y-0.5 hover:border-black/30 hover:shadow-lg dark:border-white/10 dark:bg-white/[0.04]"
    >
      <div className="relative aspect-[16/9] overflow-hidden bg-[#03002C] dark:bg-card">
        <div className="absolute inset-x-0 bottom-0 flex h-2">
          {NEXT_PALETTE_DIVISIONS.map((d, i) => (
            <span key={`${d.accent}-${i}`} className="flex-1" style={{ background: d.accent }} />
          ))}
        </div>
        <div className="absolute inset-0 flex flex-col justify-between p-5 text-white">
          <div className="inline-flex items-center gap-2 text-[10px] font-semibold uppercase tracking-widest">
            <Palette size={12} /> Example deck
          </div>
          <div>
            <div className="text-[10px] uppercase tracking-widest opacity-80">
              TransPerfect NEXT 2026
            </div>
            <div className="mt-1 text-lg font-semibold leading-tight">Palette showcase</div>
          </div>
        </div>
      </div>
      <div className="border-t border-black/10 p-4 dark:border-white/10">
        <div className="text-xs text-black/60 dark:text-white/60">
          23 slides · one section per division accent
        </div>
        <span className="mt-3 inline-flex w-full items-center justify-center gap-2 rounded-full bg-primary px-4 py-2 text-sm font-medium text-primary-foreground group-hover:opacity-90">
          <Sparkles size={14} /> Open showcase
        </span>
      </div>
    </Link>
  );
...
function EmptyState() {
  return (
    <div className="rounded-3xl border border-dashed border-black/15 bg-white/60 p-12 text-center dark:border-white/15 dark:bg-white/[0.02]">
      ...
      <Link
        to="/"
        className="mt-6 inline-flex items-center gap-2 rounded-full bg-[#03002C] px-5 py-2.5 text-sm font-medium text-white hover:opacity-90 dark:bg-primary dark:text-primary-foreground"
      >
        Browse your decks
      </Link>
    </div>
