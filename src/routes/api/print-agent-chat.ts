    ...
    handlers: {
      POST: async ({ request }) => {
        ...
        if (last?.role === "user") {
          ...
          if (error) console.error("print agent_messages insert (user) failed:", error.message);
        }
        ...
        return result.toUIMessageStreamResponse({
          ...
          onError: (error) => {
            ...
            if (/not enough credits|payment_required|402/i.test(raw))
              return "AI credits are exhausted for this workspace, so the print piece could not be generated. Top up the workspace AI credits and send the brief again.";
            if (/rate.?limit|429/i.test(raw))
              return "The AI service is rate limited right now. Wait a moment and resend the brief.";
            return raw || "The print agent hit an error.";
          },
