// /social/presets — folder-style gallery of all social presets grouped by angle.

import { AppShell } from "@/components/AppShell";
import { createFileRoute, Link } from "@tanstack/react-router";
import { useMemo } from "react";
import { FolderOpen, ArrowRight, Share2 } from "lucide-react";
import {
  SOCIAL_PLAYBOOKS,
  SOCIAL_ANGLES,
  type SocialPlaybook,
  ...
  head: () => ({
    meta: [
      ...
      { property: "og:title", content: "Social presets · TransPerfect Modular" },
      {
        ...
        content:
          "Every division-scoped social preset in one folder-style gallery — fork any preset into your saved kits in one click.",
      },
...
function PresetsView() {
  const groups = useMemo(() => {
    const map = new Map<SocialAngle, SocialPlaybook[]>();
    for (const p of SOCIAL_PLAYBOOKS) {
      const list = map.get(p.angle) ?? [];
      list.push(p);
      map.set(p.angle, list);
    }
    return SOCIAL_ANGLES.map((a) => ({
      angle: a,
      items: map.get(a.id) ?? [],
  ...
  return (
    <>
      <header className="full-bleed relative -mt-6 overflow-hidden border-b border-black/5 bg-gradient-to-br from-[#003FC70a] via-white/60 to-[#EC388a15] py-14 sm:-mt-10 lg:py-20">
        <div className="mx-auto max-w-7xl">
          <div className="max-w-3xl space-y-4">
            <div className="inline-flex items-center gap-2 rounded-full border border-black/10 bg-white/80 px-3 py-1 text-[10px] font-semibold uppercase tracking-[0.18em] text-black/60">
              <FolderOpen size={12} /> Social · preset library
            </div>
      ...
      <div className="mx-auto max-w-7xl space-y-12 px-4 py-10 sm:px-6 lg:px-8">
        {groups.map(({ angle, items }) => (
          <section key={angle.id} className="space-y-4">
