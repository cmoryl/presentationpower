...
function AbView() {
  ...
  const [variants, setVariants] = useState([
    {
      name: "Control",
      palette: { primary: "#003FC7", accent: "#A1FBF9", ink: "#03002C", surface: "#F5F1EA" },
      isControl: true,
      weight: 50,
    },
    {
      name: "Variant B",
      palette: { primary: "#EC388A", accent: "#FFEB66", ink: "#03002C", surface: "#F5F1EA" },
      isControl: false,
      weight: 50,
    },
  ]);
    ...
    <div className="space-y-8">
      <section className="rounded-2xl border border-black/10 bg-white/70 p-6 backdrop-blur">
        ...
        <p className="mt-1 text-sm text-black/60">
          Test color palette variants on rendered decks. Track views, dwell, CTA clicks and
          conversions.
        </p>
        <div className="mt-4 grid gap-3 md:grid-cols-2">
          ...
          <select
            aria-label="Metric"
            value={metric}
            onChange={(e) => setMetric(e.target.value as StatKey)}
            className="rounded-lg border border-black/15 bg-white px-3 py-2 text-sm"
          >
                ...
                {(["primary", "accent", "ink", "surface"] as const).map((k) => (
                  <label key={k} className="flex items-center gap-2 text-xs">
                    ...
                    <input
                      ...
                      value={v.palette[k]}
                      onChange={(e) =>
                        update(i, { palette: { ...v.palette, [k]: e.target.value } })
                      }
        ...
        {createM.error && (
          <div className="mt-2 text-sm text-red-600">{(createM.error as Error).message}</div>
        )}
      </section>
                ...
                <div className="flex flex-wrap items-baseline justify-between gap-2">
                  <div>
                    ...
                    <div className="text-xs text-black/50">
                      {e.brand_id ?? "all brands"} · primary: {e.primary_metric} · created{" "}
                      {new Date(e.created_at).toLocaleDateString()}
                    </div>
                    {e.hypothesis && (
                      <div className="mt-1 text-xs text-black/60 italic">{e.hypothesis}</div>
                    )}
                  </div>
                  <div className="flex flex-wrap items-center gap-1">
                    <span
                      className={`rounded-full px-2 py-0.5 text-[10px] uppercase tracking-widest ${e.status === "running" ? "bg-emerald-100 text-emerald-800" : e.status === "paused" ? "bg-amber-100 text-amber-800" : e.status === "ended" ? "bg-black/10 text-black/60" : "bg-blue-100 text-blue-800"}`}
                    >
                      {e.status}
                    </span>
                    {(["draft", "running", "paused", "ended"] as const).map((st) => (
                      <button
                        key={st}
                        onClick={() => statusM.mutate({ id: e.id, status: st })}
                        className="rounded border border-black/15 bg-white px-2 py-1 text-[10px] uppercase tracking-widest hover:bg-black/5"
                      >
                        {st}
                      </button>
                    ))}
                    <button
                      onClick={() => {
                        if (confirm(`Delete "${e.name}"?`)) delM.mutate(e.id);
                      }}
                      className="rounded border border-red-200 px-2 py-1 text-[10px] uppercase tracking-widest text-red-700 hover:bg-red-50"
                    >
                      delete
                    </button>
                  </div>
                ...
                <div className="mt-4 grid gap-3 md:grid-cols-2 lg:grid-cols-4">
                  {(e.variants as any[]).map((v) => {
                    ...
                    return (
                      <div
                        key={v.variantId}
                        className={`rounded-xl border p-3 ${isWinner ? "border-emerald-400 bg-emerald-50/50" : "border-black/10 bg-white"}`}
                      >
                        <div className="flex items-center justify-between">
                          <div className="text-xs font-medium">
                            {v.name}{" "}
                            {v.isControl && <span className="text-black/40">(control)</span>}
                          </div>
                          {isWinner && (
                            <span className="rounded bg-emerald-500 px-1.5 py-0.5 text-[9px] uppercase text-white">
                              Leader
                            </span>
                          )}
                        </div>
                        ...
                        <div className="mt-3 grid grid-cols-2 gap-1 text-[11px]">
                          <div>
                            <span className="text-black/50">Views</span> <b>{v.views}</b>
                          </div>
                          <div>
                            <span className="text-black/50">CTA</span> <b>{v.ctaClicks}</b>
                          </div>
                          <div>
                            <span className="text-black/50">Dwell</span> <b>{v.dwellAvg}ms</b>
                          </div>
                          <div>
                            <span className="text-black/50">Conv</span> <b>{v.conversions}</b>
                          </div>
                          <div className="col-span-2">
                            <span className="text-black/50">CTR</span> <b>{ctr.toFixed(1)}%</b>
                          </div>
                        </div>
                      </div>
                    );
