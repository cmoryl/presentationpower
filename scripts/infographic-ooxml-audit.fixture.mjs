...
const CASES = {
  ...
  "prst-known": (x) =>
    x.replace(/(<p:sp>[\s\S]*?<a:prstGeom[^>]*prst=")[a-zA-Z0-9]+/, "$1wobblyBlob"),
  "ext-positive": (x) => x.replace(/(<p:sp>[\s\S]*?<a:ext cx=")\d+(" cy=")\d+/, "$10$20"),
  "id-valid": (x) =>
    x.replace(/<p:cNvPr id="(\d+)" name="Text 0"/, '<p:cNvPr id="0" name="Text 0"'),
  ...
  "wrap-contract": (x) =>
    x
      .replace(/<a:bodyPr wrap="none"/, '<a:bodyPr wrap="square"')
      .replace(
        /<\/a:p><\/p:txBody>/,
  ...
  "empty-run": (x) =>
    x.replace(
      /<\/a:p><\/p:txBody>/,
      '</a:p><a:p><a:r><a:rPr lang="en-US"><a:latin typeface="Geist"/></a:rPr><a:t></a:t></a:r></a:p></p:txBody>',
    ),
