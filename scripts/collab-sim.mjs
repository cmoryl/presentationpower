...
console.log(
  `NOTE: only one real auth user exists in this project; RLS tested via SET LOCAL request.jwt.claims JWT impersonation (real policy evaluation, not service-role bypass).`,
);
...
try {
  const r1 = await sa
    ...
    .insert({
      deck_id: DECK_ID,
      author_id: OWNER_ADMIN,
      body: "SIM: deck-level comment",
      slide_index: null,
    })
    .select("id")
    .single();
  ...
  rec("Comment inserts", false, e.message);
}
...
try {
  ...
  rec("Delete reply", false, e.message);
}

// Tree
const tree = await sa
  .from("deck_comments")
  .select("id, body, slide_index, resolved, parent_id")
  .eq("deck_id", DECK_ID)
  .order("created_at");
console.log(`\nResulting comment tree (${tree.data?.length ?? 0} rows):`);
for (const c of tree.data ?? []) {
  console.log(
    `  - [${c.id.slice(0, 8)}] slide=${c.slide_index} resolved=${c.resolved} parent=${c.parent_id?.slice(0, 8) ?? "-"} :: ${c.body}`,
  );
}
...
await checkPolicy(
  "READ policy — non-admin-owner (not real owner) ⇒ false",
  NON_ADMIN_OWNER,
  READ_POLICY,
  false,
);
...
async function guardedSetStatus(actorId, next, note) {
  ...
  if (note && (next === "approved" || next === "changes_requested")) {
    await sa.from("deck_comments").insert({
      deck_id: DECK_ID,
      author_id: actorId,
      body: `[${next === "approved" ? "Approved" : "Changes requested"}] ${note}`,
    });
  }
...
await sa
  .from("decks")
  .update({ review_status: "draft", reviewed_by: null, reviewed_at: null, review_note: null })
  .eq("id", DECK_ID);
...
try {
  ...
  rec("Owner: draft → in_review", false, e.message);
}
...
try {
  ...
  const ok =
    ...
    data.reviewed_by === OWNER_ADMIN &&
    !!data.reviewed_at &&
    (noteRow.data?.length ?? 0) > 0;
  rec(
    "Admin: approve + auto-post note comment",
    ok,
    `status=${data.review_status} reviewed_by=${data.reviewed_by?.slice(0, 8)} note_comment=${noteRow.data?.[0]?.body ?? "MISSING"}`,
  );
} catch (e) {
  rec("Admin: approve", false, e.message);
}
...
try {
  ...
  rec(
    "Outsider submit-for-review REJECTED",
    /Only the owner or an admin/.test(e.message),
    e.message,
  );
}
...
for (const r of results)
  console.log(
    `${r.pass ? "PASS" : "FAIL"}\t${r.name}${r.detail ? "\t" + r.detail.slice(0, 120) : ""}`,
  );
