...
for (const m of mapped) {
  try {
    const e = buildSlideEvidence(m.source);
    ev += e ? 1 : 0;
  } catch (err) {
    console.log("evidence fail", m.source.index, (err as Error).message);
  }
}
...
for (const s of designed) {
  ...
  if (a.missing.length)
    drops.push({
      ...
      band: a.band,
      variant: s.variantId,
      cont: s.continuations?.length ?? 0,
      missing: a.missing.slice(0, 4),
    });
}
...
const low = designed
  .map((s) => ({ s, a: scoreSlideAccuracy(s) }))
  .filter((x) => x.a.band !== "high");
console.log("\n--- non-high slides:", low.length);
for (const { s, a } of low) {
  console.log(
    `#${s.source.index + 1} ${a.score} ${s.variantId} :: ` +
      a.facets.map((f) => `${f.id}=${Math.round(f.score * 100)}`).join(" ") +
      (a.missing.length ? ` | missing ${a.missing.length}` : ""),
  );
}
