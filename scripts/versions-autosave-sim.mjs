...
const { data: originalSlides } = await sa
  .from("deck_slides")
  .select("*")
  .eq("deck_id", DECK_ID)
  .order("position");
const { data: originalVersions } = await sa
  .from("deck_versions")
  .select("id")
  .eq("deck_id", DECK_ID);
console.log(
  `Owner: ${OWNER}  Deck: ${DECK_ID}  title="${originalDeck.title}" slides=${originalSlides.length} versions=${originalVersions.length}`,
);
  ...
  const { error: seedErr } = await sa.from("deck_slides").insert([
    {
      ...
      position: 0,
      section_id: "SF-01",
      variant_id: "MV-OP-COVER",
      layout_id: "LF-01",
      content: { headline: "V1 headline" },
      notes: null,
    },
    {
      deck_id: DECK_ID,
...
async function snapshotVersion(changeSummary) {
  ...
  const { data: slides } = await sa
    .from("deck_slides")
    .select("*")
    .eq("deck_id", DECK_ID)
    .order("position");
  const { data: last } = await sa
    .from("deck_versions")
...
async function restoreVersion(versionId) {
  ...
  const { data: currentSlides } = await sa
    .from("deck_slides")
    .select("*")
    .eq("deck_id", DECK_ID)
    .order("position");
  const { data: lastNum } = await sa
    .from("deck_versions")
    .select("version_number")
    .eq("deck_id", DECK_ID)
    .order("version_number", { ascending: false })
  ...
  const { data: preCheckpoint } = await sa
    .from("deck_versions")
    .insert({
      deck_id: DECK_ID,
  ...
  await sa
    ...
    .update({
      ...
      context: snapshot.deck.context ?? null,
    })
    .eq("id", DECK_ID);
  // 3. replace slides
  ...
  const { data: nextAfter } = await sa
    ...
    .limit(1)
    .maybeSingle();
  const { data: restoreLog } = await sa
    .from("deck_versions")
    .insert({
      deck_id: DECK_ID,
      version_number: (nextAfter?.version_number ?? 0) + 1,
      snapshot,
...
try {
  ...
  const { data: row } = await sa
    .from("deck_versions")
    .select("id, version_number, change_summary, created_by, snapshot")
    .eq("id", v1.id)
    .single();
  const snap = row.snapshot;
  const ok =
    row.version_number >= 1 &&
    row.change_summary === "SIM: initial state" &&
    row.created_by === OWNER &&
    snap.deck?.id === DECK_ID &&
    Array.isArray(snap.slides) &&
    snap.slides.length === 2 &&
    snap.slides[0].content.headline === "V1 headline";
  rec(
    "Snapshot v1 with correct version_number + snapshot jsonb + change_summary + created_by",
    ok,
    `v#${row.version_number} slides=${snap.slides.length} summary="${row.change_summary}"`,
  );
} catch (e) {
  rec("Snapshot v1", false, e.message);
}
...
try {
  ...
  rec(
    "List versions ordered DESC with metadata",
    ordered && hasMeta,
    `count=${list.length} top=v${list[0]?.version_number}`,
  );
} catch (e) {
  rec("List versions", false, e.message);
}
...
try {
  ...
  const beforeTitle = (await sa.from("decks").select("title").eq("id", DECK_ID).single()).data
    .title;
  const beforeSlides = (
    await sa.from("deck_slides").select("content").eq("deck_id", DECK_ID).order("position")
  ).data;
  console.log(
    `  pre-restore deck.title="${beforeTitle}" slide[0].headline="${beforeSlides[0].content.headline}"`,
  );
  ...
  const reverted =
    afterTitle === "Pulse Fest · Community Event Kit" &&
    afterSlides.length === 2 &&
    afterSlides[0].content.headline === "V1 headline";
  rec(
    "Restore reverts deck content to v1 snapshot",
    reverted,
  ...
  const { data: preRow } = await sa
    .from("deck_versions")
    .select("snapshot, change_summary")
    .eq("id", preCheckpoint.id)
    .single();
  const captured =
    preRow.snapshot.deck.title === "SIM: V2 title" &&
  ...
  rec(
    "Restore auto-checkpoints PRE-RESTORE state (nothing lost)",
    captured,
    `checkpoint.title="${preRow.snapshot.deck.title}" summary="${preRow.change_summary}"`,
  );

  // C. Restore log entry exists
  const { data: logRow } = await sa
    .from("deck_versions")
    .select("change_summary")
    .eq("id", restoreLog.id)
    .single();
  rec(
    "Restore logged as its own version entry",
    logRow.change_summary === `Restored from v${v1.version_number}`,
    `summary="${logRow.change_summary}"`,
  );
} catch (e) {
  rec("Restore v1", false, e.message);
}
...
rec(
  "Retention rule: keep last 50 OR last 7 days (whichever larger)",
  retentionOk,
  "verified by reading pruneVersions() in deck-versions.functions.ts:35-57",
);
...
try {
  ...
  rec(
    "deck_versions INSERT/RESTORE — outsider ⇒ false",
    outsiderInsert === false,
    `got ${outsiderInsert}`,
  );
} catch (e) {
  rec("deck_versions RLS", false, e.message);
}

// ================== AUTOSAVE ==================
console.log(`\n=== AUTOSAVE ===`);
try {
  const beforeUpdatedAt = (await sa.from("decks").select("updated_at").eq("id", DECK_ID).single())
    .data.updated_at;
  const beforeVersionCount = (
    await sa
      .from("deck_versions")
      .select("id", { count: "exact", head: true })
      .eq("deck_id", DECK_ID)
  ...
  const midDeck = (await sa.from("decks").select("title, updated_at").eq("id", DECK_ID).single())
    .data;
  const midVersionCount = (
    await sa
      .from("deck_versions")
      .select("id", { count: "exact", head: true })
      .eq("deck_id", DECK_ID)
  ...
  rec(
    "Autosave updates the existing decks row (title changes)",
    midDeck.title === "SIM: autosave-title-1",
    `title="${midDeck.title}"`,
  );
  rec(
    "Autosave updated_at advances",
    new Date(midDeck.updated_at).getTime() > new Date(beforeUpdatedAt).getTime(),
    `${beforeUpdatedAt} → ${midDeck.updated_at}`,
  );
  rec(
    "Autosave does NOT create a deck_versions snapshot",
    midVersionCount === beforeVersionCount,
    `versions ${beforeVersionCount} → ${midVersionCount}`,
  );
  ...
  rec(
    "Repeated autosave stays silent (no snapshot on second edit)",
    afterVersionCount === beforeVersionCount,
    `versions still ${afterVersionCount}`,
  );
} catch (e) {
  rec("Autosave", false, e.message);
}
...
try {
  ...
  rec("decks UPDATE RLS", false, e.message);
}

// ================== CLEANUP ==================
console.log(`\n=== CLEANUP ===`);
// Delete every SIM version we created
const { data: simVers } = await sa
  .from("deck_versions")
  ...
  .not(
    "id",
    "in",
    `(${(originalVersions ?? []).map((r) => `"${r.id}"`).join(",") || `"00000000-0000-0000-0000-000000000000"`})`,
  );
if (simVers?.length) {
  await sa
    .from("deck_versions")
...
await sa
  ...
  .update({
    ...
    brand_mode_id: originalDeck.brand_mode_id,
    archetype_id: originalDeck.archetype_id,
    context: originalDeck.context,
...
console.log(
  `Restored deck title, brand fields, updated_at, and ${originalSlides.length} slides (seeded slides were left as-is since deck had 0 originally? — see final check below).`,
);
...
const finalSlides = (
  await sa.from("deck_slides").select("id", { count: "exact", head: true }).eq("deck_id", DECK_ID)
).count;
const finalVers = (
  await sa.from("deck_versions").select("id", { count: "exact", head: true }).eq("deck_id", DECK_ID)
).count;
console.log(
  `Final state: title="${finalDeck.title}" slides=${finalSlides} versions=${finalVers} (should match preflight: title="${originalDeck.title}" slides=${originalSlides.length} versions=${originalVersions.length})`,
);
...
for (const r of results)
  console.log(
    `${r.pass ? "PASS" : "FAIL"}\t${r.name}${r.detail ? "\t" + r.detail.slice(0, 140) : ""}`,
  );
